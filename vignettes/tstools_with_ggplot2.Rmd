...
and hourly frequency -- each renders on the appropriate x-axis (a date axis
for daily/weekly data, a datetime axis for hourly data, and a numeric
decimal-year axis otherwise). Daily/weekly/hourly axes pick sensible tick
spacing automatically based on how much data there is (`axis_x_date_ticks`
theme parameter); the numeric axis is always year-spaced.
...
# fill_year_with_nas defaults to TRUE, padding the axis out to the entire
# calendar year the data falls in -- turned off here since this example
# only spans a few days and the padding would just show mostly empty space.
# The small margin left after the last data point scales automatically
# with how much data there is; pass axis_x_pad (in years, 0 for none) to
# override it directly. The x-axis itself is a real datetime axis, so it
# gets real day labels (e.g. "Jan 01") automatically rather than needing
# any of the above to show something meaningful.
...
For `ts`/monthly/quarterly/annual series, `highlight_window_start`/
`highlight_window_end` take a `c(year, period)` pair, as shown earlier in
the "Highlight windows: mark a period" section. That convention doesn't
apply once the x-axis is a Date or datetime (daily/weekly/hourly xts) --
instead, pass a `Date`/`POSIXct` value directly, or anything
`as.Date()`/`as.POSIXct()` accepts, like a `"YYYY-MM-DD"` string.
...
`tsr` (right-axis) series is present. `tsggplotly()` cannot convert the split
left/right legend described above, so a `tsr` plot headed for
`tsggplotly()` needs `legend_all_left = TRUE`; `tsggplotly()` errors with a
reminder to that effect if you forget. Note that `legend_all_left` won't
visibly change anything about the interactive legend itself -- `tsggplotly()`
always renders a single merged legend regardless, since it has no way to
draw the split one at all; the parameter's only job here is making the plot
...
For longer series, `tsggplotly()` avoids overlapping yearly x-axis labels the
same way the static plot does (blanking out labels that would collide, while
still drawing every tick mark). If you'd rather Plotly pick "nice" tick
positions dynamically as the plot is resized or zoomed -- at the cost of no
longer aligning ticks to exact year starts -- pass `x_tick_mode = "auto"`.
This also gives Plotly a real date-typed x-axis, so as you zoom in the tick
labels switch to months/days instead of staying decimal years like `2020.5`.
